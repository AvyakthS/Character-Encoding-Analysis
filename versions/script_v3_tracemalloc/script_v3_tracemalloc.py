...
print("Running encoding analysis V3 (with tracemalloc)... This may take a moment.\n")

# --- Test Suite 1: File I/O (Disk-Bound) ---
# This test is unchanged. 'psutil' is still fine for a high-level
# view of the entire I/O operation.
...
for test_name, source_file in datasets_to_test:
    ...
    for encoding in encodings_map[test_name]:
        ...
        try:
            ...
            # Get peak memory allocated *during* the encode loop
            _, encode_mem_peak = tracemalloc.get_traced_memory() # <-- GET MEM
            tracemalloc.stop() # <-- STOP
            tracemalloc.clear_traces() # <-- CLEAR
            ...
            # Get peak memory allocated *during* the decode loop
            _, decode_mem_peak = tracemalloc.get_traced_memory() # <-- GET MEM
            tracemalloc.stop() # <-- STOP
            tracemalloc.clear_traces() # <-- CLEAR

            avg_decode_time = (end_decode_time - start_decode_time) / ITERATIONS

            # --- 3. Get System Stats (CPU Load only) ---
            cpu_load = process.cpu_percent(interval=None)
            ...
            cpu_results.append({
                ...
                "Avg CPU Load (%)": cpu_load,
                "Encode Peak Mem (MB)": encode_mem_peak / (1024 * 1024), # <-- NEW METRIC
                "Decode Peak Mem (MB)": decode_mem_peak / (1024 * 1024)  # <-- NEW METRIC
            })

        except Exception as e:
            tracemalloc.stop() # Ensure it stops on error
            ...
            cpu_results.append({
                ...
                "Avg CPU Load (%)": "N/A",
                "Encode Peak Mem (MB)": "N/A",
                "Decode Peak Mem (MB)": "N/A"
            })
...
if cpu_results:
    # This will now auto-include the new memory columns
...
if io_results:
    ...
    for res in io_results:
        # Added try/except for formatting in case of "N/A"
        try:
            w_time = f"{res['Avg. Write Time (s)']:<22.8f}"
            r_time = f"{res['Avg. Read Time (s)']:<22.8f}"
            cpu_load = f"{res['Avg CPU Load (%)']:<15.2f}"
            ram_delta = f"{res['RAM Delta (MB)']:<15.4f}"
        except (ValueError, TypeError):
            w_time = f"{res['Avg. Write Time (s)']:<22}"
            r_time = f"{res['Avg. Read Time (s)']:<22}"
            cpu_load = f"{res['Avg CPU Load (%)']:<15}"
            ram_delta = f"{res['RAM Delta (MB)']:<15}"

        print(f"{res['Dataset']:<23} | {res['Encoding']:<10} | {str(res['File Size (Bytes)']):<18} | {w_time} | {r_time} | {cpu_load} | {ram_delta}")

if cpu_results:
    print("\n--- FINAL RESULTS (In-Memory CPU) ---")
    print(f"{'Dataset':<23} | {'Encoding':<10} | {'Avg. Encode Time (s)':<22} | {'Avg. Decode Time (s)':<22} | {'Avg CPU (%)':<15} | {'Encode Peak (MB)':<18} | {'Decode Peak (MB)':<18}")
    print("-" * 135)
    for res in cpu_results:
        # Added try/except for formatting in case of "N/A"
        try:
            e_time = f"{res['Avg. Encode Time (s)']:<22.8f}"
            d_time = f"{res['Avg. Decode Time (s)']:<22.8f}"
            cpu_load = f"{res['Avg CPU Load (%)']:<15.2f}"
            e_mem = f"{res['Encode Peak Mem (MB)']:<18.4f}"
            d_mem = f"{res['Decode Peak Mem (MB)']:<18.4f}"
        except (ValueError, TypeError):
            e_time = f"{res['Avg. Encode Time (s)']:<22}"
            d_time = f"{res['Avg. Decode Time (s)']:<22}"
            cpu_load = f"{res['Avg CPU Load (%)']:<15}"
            e_mem = f"{res['Encode Peak Mem (MB)']:<18}"
            d_mem = f"{res['Decode Peak Mem (MB)']:<18}"
            
        print(f"{res['Dataset']:<23} | {res['Encoding']:<10} | {e_time} | {d_time} | {cpu_load} | {e_mem} | {d_mem}")