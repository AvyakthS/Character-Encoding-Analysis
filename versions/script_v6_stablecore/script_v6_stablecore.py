...
# --- Core Logic Functions (Identical, but will be called by main) ---
...
# --- Test 4: Final Analysis (V6 REWORK) ---
def run_analysis(io_results, cpu_results):
    """Reworked to build clean, un-pivoted tables."""
    io_analysis_results = []
    ...
    for dataset in unique_datasets:
        ...
        for row in io_rows:
            ...
            io_analysis_results.append({
                ...
                "File Size (B)": row['File Size (Bytes)'],
        ...
        for row in cpu_rows:
            ...
            # Handle potential divide by zero for mem baseline
...
# --- REFACTORED V7 PRINTING FUNCTIONS (Take logger) ---
...
def print_beautiful_box(logger, title, content, indent_level=2):
    ...
    max_width = max(len(line) for line in content_lines if line)
...
def main():
    ...
    for test_name, source_file in datasets_to_test:
        print_beautiful_header(logger, f"Loading Dataset: {test_name} ({source_file})")
        try:
        ...
        logger.log(f"{BASE_INDENT * 2}Running Test Suite 1: File I/O (Disk-Bound)...")
    ...
    for dataset in unique_datasets:
        ...
        print_beautiful_table(logger, f"Dataset: {dataset}", rows)
    ...
    if profiler_reports:
        print_beautiful_box(logger, "Profiler Report for: utf-8 (English Dataset)", profiler_reports['utf-8'])
    else:
